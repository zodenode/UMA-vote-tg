...
| `apps/bot` | Grammy bot: `/start` (optional welcome photo), **📖 How it works** (`/how`, optional diagram via `HOW_IT_WORKS_PHOTO_URL`), inline menus, alerts, group commands |
...
1. Copy [`.env.example`](.env.example) to `.env` in the repo root (or set env per process). You need at least:
   ...
   - Optional **`HOW_IT_WORKS_PHOTO_URL`** or **`HOW_IT_WORKS_PHOTO_FILE_ID`** on the **bot** so **📖 How it works** and **`/how`** send your diagram above the step-by-step caption (otherwise the same text is sent without an image)
